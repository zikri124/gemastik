...
const gigByStatus = async (req, res, next) => {
    ...
    if (status != 'done') {
        const [query] = await db.query('select * from gigs where gigStatus = ? and ownerId = ?', [status, userId]) //order by createdAt desc
        if(row.length>0){
            res.json({
                "gigs" : query
            })
        ...
        const [query] = await db.query('select * from histories where ownerId = ?', [userId]) 
        if(row.length>0){
            res.json({
                "gigs" : query
            })
